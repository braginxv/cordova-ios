...
- (void)pluginInitialize {
    ...
    self.uiDelegate = [[CDVWKWebViewDelegate alloc] initWithTitle:[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"]];
...
- (id)loadRequest:(NSURLRequest *)request {
    if ([self canLoadRequest:request]) { // can load, differentiate between file urls and other schemes
        if (request.URL.fileURL) {
            ...
            return [(id) _engineWebView loadFileURL:request.URL allowingReadAccessToURL:readAccessUrl];
        } else {
...
decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction
    ...
    for (NSString *pluginName in vc.pluginObjects) {
        ...
        if ([plugin respondsToSelector:selector]) {
            ...
            if((((BOOL (*)(id, SEL, id, int))objc_msgSend)(plugin, selector, navigationAction.request, navType))) {
     ...
     */
    if ([self defaultResourcePolicyForURL:url]) {
        return decisionHandler(WKNavigationActionPolicyAllow);
    } else {
